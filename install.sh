...
if [[ -d "${INSTALL_DIR}" && -f "${INSTALL_DIR}/package.json" ]]; then
    ...
    # Rename staging to a stable temp name BEFORE touching the current install.
    # This ensures that if anything fails after this point, $STAGING_DIR (now
    # $_NEW_DIR) still exists and the system is not left without an install dir.
    ...
    if [[ -d "$INSTALL_DIR" ]]; then
        ...
        # node_modules is ~200 MB and fully regenerated by npm install — no need to back it up
    ...
    # Keep only the 2 most recent backups — prevents SD card from filling up on repeated installs
...
# Expand swap to 1 GB before compiling native C++ modules.
# better-sqlite3 + node-pty compiled in parallel can peak at ~500 MB RAM,
# which kills SSH/system processes via OOM on 1–2 GB Raspberry Pi models.
_SWAP_EXPANDED=0
_ORIG_SWAP=0
if [[ -f /etc/dphys-swapfile ]] && command -v dphys-swapfile &>/dev/null; then
    ...
    if [[ "$_ORIG_SWAP" -lt 1024 ]]; then
        info "Temporarily expanding swap to 1024 MB for native module compilation..."
        sed -i "s/^CONF_SWAPSIZE=.*/CONF_SWAPSIZE=1024/" /etc/dphys-swapfile
        if dphys-swapfile setup >/dev/null 2>&1 && dphys-swapfile swapon >/dev/null 2>&1; then
            _SWAP_EXPANDED=1
            ok "Swap expanded to 1024 MB"
        else
            # Restore original value if expansion failed
            sed -i "s/^CONF_SWAPSIZE=.*/CONF_SWAPSIZE=${_ORIG_SWAP}/" /etc/dphys-swapfile
            warn "Could not expand swap — compilation may fail on low-RAM devices"
        fi
    fi
fi

# --jobs=1 serializes C++ compilation to avoid parallel OOM on Raspberry Pi
if ! npm install --loglevel=error --jobs=1 2>/tmp/homepinas-npm.log; then
    # Restore swap before exiting so the system is left in a clean state
    if [[ "$_SWAP_EXPANDED" -eq 1 ]]; then
        sed -i "s/^CONF_SWAPSIZE=.*/CONF_SWAPSIZE=${_ORIG_SWAP}/" /etc/dphys-swapfile
...
# Stop restarting after 5 failures in 60 seconds — prevents CPU/RAM thrash loop on Pi
StartLimitBurst=5
...
# Cap Node.js heap at 256 MB — tsx transpiles TypeScript on-the-fly and can
# exhaust RAM on Raspberry Pi without this limit, causing repeated OOM crashes
