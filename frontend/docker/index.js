...
import { openComposeModal, saveCompose, runCompose, stopCompose, deleteCompose, openEditComposeModal } from './compose.js';
import { buildContainerCard, buildComposeCard, openStacksManager } from './containers.js';
...
async function renderDockerManager() {
    ...
    if (state.dockers.length === 0) {
        ...
        state.dockers.forEach(container => {
            containerGrid.appendChild(buildContainerCard(container));
        });
    ...
    if (composeFiles.length > 0) {
        ...
        composeFiles.forEach(compose => {
            composeGrid.appendChild(buildComposeCard(compose, runCompose, stopCompose, deleteCompose));
        });
